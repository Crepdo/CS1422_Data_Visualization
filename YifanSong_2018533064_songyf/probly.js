const probly = `Almost Certainly,Highly Likely,Very Good Chance,Probable,Likely,Probably,We Believe,Better Than Even,About Even,We Doubt,Improbable,Unlikely,Probably Not,Little Chance,Almost No Chance,Highly Unlikely,Chances Are Slight
...
var problyData = d3.csvParse(probly);
var canvas2 = d3.select("#chart2");
var margin = { top: 100, right: 10, bottom: 80, left: 100 };
...
temp_key_list = [];
for (var k in problyData[0]) {
    temp_key_list.push(k);
};
key_list = []
for (var i = temp_key_list.length - 1; i >= 0; i--) {
    key_list.push(temp_key_list[i]);
}
...
for (var i = key_list.length - 1; i >= 0; i--) {
    var data = problyData.map(function (d) { return d[key_list[i]] });
    var density = kde(data);
    svg.append("path")
        .attr("transform", function (d) { return ("translate(0," + (yName(key_list[i]) - height) + ")") })
        .datum(density)
        .attr("fill", function (d) {
            mean = d3.mean(data)
            console.log(mean);
            return problyColor(mean)
        })
        .attr("opacity", ".8")
        .attr("stroke", "#000")
        .attr("stroke-width", 1)
